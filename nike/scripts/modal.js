export default class Modal {
  ...
  onDocumentClick(event) {
    const targetButton = event.target.closest("[data-js-modal-button]");

    if (targetButton) {
      const target = targetButton.dataset.jsModalButton;
      const speed = targetButton.dataset.jsModalSpeed;

      this.lastActiveElement = document.activeElement;
      this.speed = speed ? parseInt(speed) : 300;
      this.modalWindow = document.querySelector(
        `[data-js-modal-window="${target}"]`
      );

      return this.open();
    }

    const targetClose = event.target.closest("[data-js-modal-close]");
  ...
  onModalClick(event) {
    ...
    if (!target.closest("[data-js-modal-window]") && this.isOpen) {
      this.close();
    }
  }
